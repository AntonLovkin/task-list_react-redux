...
function Notes() {
    ...
    return (<div>
        <ul>
            <NotesHeader />
            {activeNotes && activeNotes.length > 0 && activeNotes.map(note => <Note note={note} editBtnClick={editBtnClick} />)}
        </ul>

        <button onClick={handleClick}>Add note</button>
        {showForm && type === FORM_TYPES.ADD && <Form note={null} />}
        {showForm && type === FORM_TYPES.EDIT && <Form note={noteData} />}

        {archivedNotes && archivedNotes.length > 0 && <div>
            <h2>Archived notes list</h2>
            <ul>
                {archivedNotes.map(note => <Note note={note} editBtnClick={editBtnClick} />)}
            </ul>
        </div>}
    </div>);
}
